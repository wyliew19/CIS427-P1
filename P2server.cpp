...
struct user {
    ...
    user() {
        this->fd = -1;
        this->username = (char*)malloc(sizeof(char)*12);
        this->id = -1;
    }
};
...
// Author: Will Wylie
void lookup_request(int client_socket, sqlite3* db, const char* request, int owner_id) {
    char search_name[MAX_LINE];
    char response[MAX_LINE * 10];
    response[0] = '\0';
    char sql_query[MAX_LINE];
    char* zErrMsg = 0;

    // Parse the LOOKUP command request
    if (sscanf(request, "LOOKUP %s", search_name) != 1) {
        sprintf(response, "400 Bad Request\nInvalid LOOKUP command format\n");
        send(client_socket, response, strlen(response), 0);
        return;
    }

    // Construct the SQL query to retrieve matching card records
    sprintf(sql_query, "SELECT ID, card_name, card_type, rarity, count, owner_id FROM Pokemon_Cards WHERE owner_id=%d AND card_name LIKE '%%%s%%';", owner_id, search_name);
    ...
    if (rc != SQLITE_OK) {
        fprintf(stderr, "SQL error: %s\n", zErrMsg);
        sqlite3_free(zErrMsg);
        sprintf(response, "500 Internal Server Error\nCould not retrieve card records.\n");
        send(client_socket, response, strlen(response), 0);
        return;
    }
    ...
    if (strlen(response) == 0) {
        ...
        // Matching records found, prepend the response with "200 OK"
        char full_response[MAX_LINE * 10];
        snprintf(full_response, sizeof(full_response), "200 OK\nFound %d match%s\n%s", rc, (rc == 1) ? "" : "es", response);
        strcpy(response, full_response);
    }
...
void login(int client_socket, sqlite3* db, user& client) {
    ...
    char query[256];
    snprintf(query, sizeof(query), "SELECT ID,user_name FROM users WHERE user_name='%s' AND password='%s';", username, password);

    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, query, -1, &stmt, NULL) != SQLITE_OK) {
        char* zErrMsg = (char*)sqlite3_errmsg(db);
        fprintf(stderr,"SQLite error: %s%s", zErrMsg, "\n");
        close(client_socket);
        return;
    }
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        char* response = "401 wrong username/password\n\0";
        send(client_socket, response, strlen(response), 0);
        client = -1;
        close(client_socket);
    }
...
// Author: Will Wylie
void who_request(int client_socket, user* client_sockets) {
    ...
    // Iterate through client_sockets to find active users
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (client_sockets[i].fd != -1) {
            // Append the username and IP address to the response
            char user_ip[MAX_LINE];
            char client_ip[INET_ADDRSTRLEN];
            struct sockaddr_in addr;
            socklen_t addr_len = sizeof(addr);
            getpeername(client_sockets[i].fd, (struct sockaddr*)&addr, &addr_len);
            inet_ntop(AF_INET, &(addr.sin_addr), client_ip, INET_ADDRSTRLEN);

            snprintf(user_ip, sizeof(user_ip), "%s %s\n", client_sockets[i].username, client_ip);
            strcat(response, user_ip);
            num_active_users++;
        }
    }

    if (num_active_users > 0) {
        // If there are active users, send the list
        char full_response[MAX_LINE * 10];
        snprintf(full_response, sizeof(full_response), "200 OK\nThe list of active users:\n%s", response);
        send(client_socket, full_response, strlen(full_response), 0);
    } else {
        // No active users to list
        char no_users_response[] = "200 OK\nNo active users.\n";
        send(client_socket, no_users_response, strlen(no_users_response), 0);
    }
}
            ...
            if (client_socket != -1 && FD_ISSET(client_socket, &read_fds)) {
                if (client_sockets[i].username[0] == '\0') {
                    ...
                    if (valread <= 0) {
                        ...
                        if (strncmp(req, "BUY", 3) == 0) {
                        ...
                        } else if (strncmp(req, "LOOKUP", 6) == 0) {
                            lookup_request(client_socket, db, req, client_sockets[i].id);
                        } else if (strncmp(req, "DEPOSIT", 7) == 0) {
                            deposit_request(client_socket, db, req);
                            ...
                            if (client_sockets[i].id == ROOT_USERID)
                                ...
                                sprintf(buf, "401 only root user can use SHUTDOWN\n\0");
                                send(client_socket, buf, strlen(buf), 0);
                            }
                        } else if (strncmp(req, "WHO", 3) == 0) {
                            if (client_sockets[i].id == ROOT_USERID)
                                who_request(client_socket, client_sockets);
                            else {
                                sprintf(buf, "401 only root user can use WHO\n\0");
                                send(client_socket, buf, strlen(buf), 0);
                            }
                        } else {
